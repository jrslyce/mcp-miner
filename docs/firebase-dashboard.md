...
- Pro users can view an in-portal weekly digest through `getWeeklyDigest`, aggregating only abstract cloud game state: Chonks, Space Bucks, materials, order readiness, sync health, milestones, base state, and cosmetic changes.
...
- Digest and beta-access preferences are stored in `/players/{uid}/settings/current` as `weeklyDigestEnabled` and `betaFeaturesEnabled`; the beta toggle controls portal experimental access without changing the underlying subscription record.
...
The smoke test signs up an Auth emulator user, sends one abstract reward event to `syncRewardEvents`, reads `getSyncState`, `getDashboardAnalytics`, `getWeeklyDigest`, and `getCosmeticCatalog`, and verifies Firebase Hosting serves the dashboard panels.
