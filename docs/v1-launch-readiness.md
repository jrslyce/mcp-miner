...
| Subscription launch QA | Ready for matrix validation | `docs/subscription-qa-matrix.md` is the launch gate for billing, entitlements, sync limits, downgrade, crypto gating, and security; `docs/subscription-ops-runbook.md` covers observability, support tooling, budget alerts, rate limits, deploy, and rollback; `test:subscription-qa`, `test:subscription-ops`, `test:support-tools`, and `test:crypto-billing` verify coverage. |
...
npm run test:support-tools
npm run test:crypto-billing
```
